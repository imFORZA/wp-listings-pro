 ...
 $level = 'clients',
	...
	public function set_wrapper( $idx_page, $wrapper_url ) {
		...
		if ( $idx_page === 'none' ) {
			...
			// set Global Wrapper:
			$this->idx_api( 'dynamicwrapperurl', $this->IDX_API_DEFAULT_VERSION, 'clients', array( 'body' => array( 'dynamicURL' => $wrapper_url ) ), 10, 'POST' ); // '1.4.0'
		} else {
			...
			$this->idx_api( 'dynamicwrapperurl', $this->IDX_API_DEFAULT_VERSION, 'clients', array( 'body' => $params ), 10, 'POST' ); // '1.4.0'
		}
	}
	...
	public function clear_wrapper_cache() {
		...
		$url = $this->IDX_API_URL . '/clients/wrappercache'; // 'https://api.idxbroker.com'
		$method = 'DELETE';
	...
	public function searchfields( $idxID ) {
		$approved_mls = $this->idx_api( "searchfields/$idxID", $this->IDX_API_DEFAULT_VERSION, 'mls', array() );

		return $approved_mls;
	}

	public function searchfieldvalues( $idxID, $fieldName, $mlsPtID ) {
		$approved_mls = $this->idx_api( "searchfieldvalues/$idxID?mlsPtID=$mlsPtID&name=$fieldName", $this->IDX_API_DEFAULT_VERSION, 'mls', array() );
